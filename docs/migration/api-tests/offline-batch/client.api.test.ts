 ...
 * The offline browse routes require a Bearer token (mock customer JWT from the
 * api-tests auth store). MySQL data path proven via tsx.
 *
  ...
  return runTests("offline-batch (client)", [
...

    {
      ...
      fn: async () => {
        ...
        const data = (await requestOk("GET", "/api/v1/client/offline/centers", { token })).data as any[];
    ...
    },
    {
      ...
      fn: async () => {
        const token = await getCustomerToken();
        const data = (await requestOk("GET", "/api/v1/client/offline/batches", { token })).data as any[];
