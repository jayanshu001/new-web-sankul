...
| **CP3.5** | **Port ALL Mongo-only functions to SQL** — user decision 2026-06-30. Plan: `docs/migration/CP3_5_PORT_PLAN.md`. Most are delete-dead-body + flip-to-existing-SQL-twin; only ~5 need DDL; no live SQL path depends on Mongo. **CP4 blocked until done.** | ⏸ PAUSED (user request 2026-06-30) — NOT started; Batch-0 agents were stopped during analysis, **0 files modified**. Resting at CP3 checkpoint (green). Resume order: 0 dead → 1 reads → 2 admin-CRUD/live/exam → 3 orders/checkout (some retire) → 4 payment+auth (needs user OK + DDL). |
