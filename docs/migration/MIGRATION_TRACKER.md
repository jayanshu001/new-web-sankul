...
> **Last updated:** 2026-06-30 — **FULL MONGO REMOVAL reopened** (see [`MONGO_REMOVAL_STATUS.md`](./MONGO_REMOVAL_STATUS.md) + [`CP3_5_PORT_PLAN.md`](./CP3_5_PORT_PLAN.md)): CP1+CP2+CP3 done (all `isMysqlModule()` fallback branches removed, build green; committed `94067c1`); CP3.5 in progress (port ~29 genuinely Mongo-only endpoints — Batch 0 dead-code-delete done). The "MIGRATION COMPLETE 2026-06-22" banner above was optimistic — those 29 live endpoints had NO SQL branch. _(prior: 2026-06-18 Waves 1–8 COMPLETE on SQL; VideoCategory DAG prereq + Mongo-only surfaces remained.)_  
> **Current phase:** Phase 2/3 — Backend stabilization. Read + write modules all built; full flip enabled in this env. — ✅ DONE (superseded; see banner) · ALL waves/modules complete, all flags ON, running MySQL-only. **2026-06-17 ADMIN + EDUCATOR wave:** admin-auth (`ws_users` + spatie roles + new `ws_admin_access_tokens`), admin administrator CRUD, admin customer CRUD (`ws_customer`), educator-auth (`ws_course_educator` + new `ws_educator_access_tokens`, **MD5+bcrypt** passwords), admin educator master CRUD — all on SQL, flags ON, verified. **2026-06-18 Mongo-only push (resumable plan: [`MONGO_ONLY_MIGRATION_PLAN.md`](./MONGO_ONLY_MIGRATION_PLAN.md)):** ✅ Waves 1–5 (Promoter/Referral/RBAC/Exam/all 9 admin catalog CRUD + client cart/educator). ✅ **Wave 6 (LiveCourse/LiveSession)** — 14 tables created + backfilled + admin/client surfaces. ✅ **Wave 7 (aggregation/finalizers)** — all aggregators + full payment (course/ebook/book/live-course/package + test-series) + webhooks + categories/my-subs/orders; 8 previously-blocked tables CREATED + backfilled. ✅ **`client-notification` FULLY migrated + flag ON** (ws_customer_device_token + admin write subsystem + dual-read BullMQ cutover). ✅ **`client-lecture-progress` free-video slice migrated + flag ON** (container/DAG paths still Mongo — need VideoCategory childCategoryIds DAG → SQL). ✅ **Wave 8 (misc/low-value) DONE 2026-06-18** — no-DDL: customer-master/ImageNotification/offline Center+Batch+Enquiry+City; DDL batch (5 new tables + 2 ALTERs): tracking(ActivityLog)/goal/cms-extra(SocialLink+Type/CurrentAffair/LiveBannerSlider)/inquiry/offline-Banner. Verified 24/24+23/23+14/14+34/34. **⏭️ ONLY REMAINING:** VideoCategory DAG → SQL (unblocks container lecture-progress + resume/learning reads), then profile-dashboard subscriptions/pastExams counts. Genuinely Mongo-only (no clean slice): ExamCountdown, PackageCategory (no table), client/dashboard, recordingWebhook, admin exam/question CRUD writes, realtime/streaming stack. See [`../MIGRATION_MONGO_REMAINING.md`](../MIGRATION_MONGO_REMAINING.md).  
